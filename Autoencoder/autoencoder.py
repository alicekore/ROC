...
def prepareDataSet():
    ...
    set_orig = numpy.array(raw)
    set = numpy.reshape(set_orig, newshape=(set_orig.shape[0], numpy.prod(set_orig.shape[1:])))
    return set
...
def autoencoder():
    ...
    # Training AE
    ae.fit(x_train, x_train, epochs=20, batch_size=256, shuffle=True, validation_data=(x_test, x_test)) # validation data <> validation! (validation set)
